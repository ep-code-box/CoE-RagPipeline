...
def upgrade() -> None:
    ...
    def has_tables(*names: str) -> bool:
        try:
            return all(inspector.has_table(n) for n in names)
        except Exception:
            return False
    ...
    try:
        ...
        pass
    if has_tables('analysis_requests'):
        try:
        ...
        try:
            ...
            pass
        for col in ['repositories','include_ast','include_tech_spec','include_correlation','created_at','updated_at']:
            try:
                op.alter_column('analysis_requests', col, nullable=True)
            except Exception:
                pass
    ...
    if has_tables('repository_analyses','analysis_requests'):
        try:
    ...
    if (not sa.inspect(conn).has_table('correlation_analyses')) and has_tables('analysis_requests','repository_analyses'):
        try:
    ...
    # document_analyses
    if (not sa.inspect(conn).has_table('document_analyses')) and has_tables('repository_analyses'):
    ...
    # tech_dependencies
    if (not sa.inspect(conn).has_table('tech_dependencies')) and has_tables('repository_analyses'):
    ...
    # ast_nodes
    if (not sa.inspect(conn).has_table('ast_nodes')) and has_tables('repository_analyses'):
